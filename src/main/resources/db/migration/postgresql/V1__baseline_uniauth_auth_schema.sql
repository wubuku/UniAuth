-- UniAuth PostgreSQL baseline.
-- Source: schema-only export of the approved blacksheep_dev auth tables.
-- Verified: 2026-08-07 against PostgreSQL 16.8.
-- Normalized source SHA-256: 6db1ad5bebad8b5dd14b6265aa0065d05799fa11eeedb8e6e88f88fd8d45aaea
-- This migration intentionally preserves the deployed structure. Hardening changes start at V2.
...
CREATE INDEX idx_login_methods_primary
    ON public.user_login_methods USING btree (user_id, is_primary);

CREATE INDEX idx_login_methods_provider
    ON public.user_login_methods USING btree (auth_provider, provider_user_id);

CREATE INDEX idx_login_methods_user_id
    ON public.user_login_methods USING btree (user_id);

CREATE INDEX idx_user_login_methods_chain_id
    ON public.user_login_methods USING btree (chain_id)
    WHERE ((auth_provider)::text = 'WEB3'::text);

CREATE INDEX idx_user_login_methods_web3_nonce
    ON public.user_login_methods USING btree (web3_nonce)
    WHERE ((auth_provider)::text = 'WEB3'::text);

CREATE UNIQUE INDEX uk_local_username
    ON public.user_login_methods USING btree (local_username)
...
CREATE UNIQUE INDEX uk_user_login_provider
    ON public.user_login_methods USING btree (user_id, auth_provider);

ALTER TABLE ONLY public.user_login_methods
    ADD CONSTRAINT user_login_methods_user_id_fkey
...
CREATE INDEX idx_web3_nonces_expires_at
    ON public.web3_nonces USING btree (expires_at);

CREATE INDEX idx_web3_nonces_wallet_address
    ON public.web3_nonces USING btree (wallet_address);

CREATE UNIQUE INDEX web3_nonces_wallet_address_key
    ON public.web3_nonces USING btree (wallet_address);
...
ALTER TABLE ONLY public.user_authorities
    ADD CONSTRAINT user_authorities_user_id_fkey
    FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;
...
CREATE INDEX idx_token_blacklist_expires_at
    ON public.token_blacklist USING btree (expires_at);
CREATE INDEX idx_token_blacklist_jti
    ON public.token_blacklist USING btree (jti);
...
CREATE UNIQUE INDEX spring_session_ix1
    ON public.spring_session USING btree (session_id);
CREATE INDEX spring_session_ix2
    ON public.spring_session USING btree (expiry_time);
CREATE INDEX spring_session_ix3
    ON public.spring_session USING btree (principal_name);
...
ALTER TABLE ONLY public.spring_session_attributes
    ADD CONSTRAINT spring_session_attributes_pkey
    PRIMARY KEY (session_primary_id, attribute_name);

ALTER TABLE ONLY public.spring_session_attributes
    ADD CONSTRAINT spring_session_attributes_session_primary_id_fkey
