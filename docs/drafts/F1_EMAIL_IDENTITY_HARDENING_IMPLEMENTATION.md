...
- F1 的所有固定范围完成并通过统一硬门槛后，F1 结束；本轮不单独执行连续三轮
  无修改检查。
- 原冻结计划曾要求 F1 后继续执行 F2-F5；该规则现已停止自动执行。F2-F5 只有在
  未来明确提出对应需求时才作为独立批次启动。
...
6. 按 F1 当时的冻结计划，完整统一门禁通过后记录 F1 验收证据并计划进入 F2，
   连续三轮无修改检查原计划延后到 F1-F5 全部完成后的阶段末执行；该后续计划现已
   停止自动执行。
...
F1 按当时阶段规则只以自动化验收收口，没有执行单轮连续三轮无修改检查。原计划曾把
整个 F 阶段的唯一一次三轮检查延后到 F5；该后续计划现已停止自动执行。F2-F5 只有在
后续明确要求时才作为独立批次启动，下面的 post-F1 V5 收尾负责本次邮箱固化任务的
最终三轮检查和退出。

### 2026-08-09 Post-F1 邮件参考服务 V5 收尾

该收尾不改写上面的 F1 V4 验收历史，也不启动 F2。邮件服务新增 forward-only V5，
...
shared-schema、HTTP、Flyway、Playwright、Python、前端和文档门槛均通过。固定范围
只剩三轮无修改检查和交付。完成后邮箱参考服务固化进度记为 100%，当前固化阶段
结束；F2-F5 不属于本次未完成项，后续直接按明确的新功能、缺陷或维护需求重新规划。

## 8. 检查记录

本文实施记录只保存 F1 和 post-F1 V5 收尾的执行与验收证据，不建立长期检查计数器。
V5 收尾的三轮无修改检查只在完整门禁通过后执行，结果在当次工作报告中输出；无问题
轮次不为留痕修改仓库文件。三轮通过并交付后即退出邮箱固化阶段。
