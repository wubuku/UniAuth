...
echo "7/15 Reject an invalid recovery scan interval"
expect_failure \
...
echo "8/15 Reject a recovery window shorter than the SMTP timeout budget"
expect_failure \
...
echo "9/15 Reject an environment file readable by group or others"
...
echo "10/15 Reject a symbolic-link environment file"
...
echo "11/15 Accept an owner-only environment file"
email_service_validate_env_file "$secure_env"

echo "12/15 Reject an API key containing a line break"
expect_failure \
...
echo "13/15 Reject an oversized API key"
...
echo "14/15 Accept loopback development with authentication disabled"
run_guard >/dev/null

echo "15/15 Accept protected non-loopback production configuration"
run_guard \
    SPRING_PROFILES_ACTIVE=prod \
    EMAIL_POSTGRES_DATABASE=email_service_prod \
    EMAIL_SERVICE_BIND_ADDRESS=0.0.0.0 \
