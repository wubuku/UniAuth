...
已完成或失效的草稿保留在原路径，并在本索引中标记为“历史”或“已完成”。
仅当仓库已经存在归档目录时，才链接到其索引；除非用户明确批准，不移动或重命名既有文件。
```
...
1. 功能完全实现、决策失效或内容被替代时，在索引中将生命周期标记为“历史”
2. 既有文档保留原文件名和原路径，避免破坏来源信息和已有链接
3. 在“替代文档”列中链接当前权威指南；不要复制旧文档的大段内容
4. 只有用户明确批准迁移时，才移动文件并同步修复所有引用

## 归档索引

| 状态日期 | 原路径文件 | 状态 | 替代文档/说明 |
|----------|------------|------|---------------|
| 2025-08-15 | [积分系统设计.md](../drafts/积分系统设计.md) | 历史 | 已实现；当前行为见正式指南 |
```
