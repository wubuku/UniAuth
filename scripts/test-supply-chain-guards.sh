...
echo "1/8 Reject a missing dependency audit report"
expect_failure \
...
echo "2/8 Accept a dependency audit report with project and dependency evidence"
printf '%s\n' \
...
echo "3/8 Reject an expired dependency suppression"
...
echo "4/8 Reject a suppression without an expiry"
...
echo "5/8 Reject a suppression without detailed notes"
...
echo "6/8 Accept the repository suppression manifests"
"$PYTHON_BIN" \
    "$SCRIPT_DIR/check-dependency-suppressions.py" \
...
echo "7/8 Reject a floating PostgreSQL image tag"
...
echo "8/8 Accept repository PostgreSQL 16.13 image pins"
"$PYTHON_BIN" \
    "$SCRIPT_DIR/check-postgres-image-pins.py" \
